    ...
    private function seedCourses(array $categories): void
    {
        $courses = [
            'category-b-manual' => [
                ...
                'practice_hours' => 30,
                'duration_weeks' => 14,
                'format' => CourseFormat::Hybrid->value,
                'icon' => 'car-front',
                'is_featured' => true,
            ],
            'category-a-motorcycle' => [
                ...
                'practice_hours' => 12,
                'duration_weeks' => 6,
                'format' => CourseFormat::Group->value,
                'icon' => 'bike',
                'is_featured' => false,
            ],
            'individual-driving-lessons' => [
                ...
                'practice_hours' => 10,
                'duration_weeks' => 3,
                'format' => CourseFormat::Individual->value,
        ...
        CourseCategory::query()
            ->chunkById(100, function ($categories): void {
                foreach ($categories as $category) {
                    $name = $this->completeTranslations(
                        $category->getTranslations('name'),
                        $this->translationsFromValue(Str::headline($category->slug ?: 'Course category')),
                    );
                    $shortDescription = $this->completeTranslations(
                        $category->getTranslations('short_description'),
                        [
                            'ru' => 'Категория курсов автошколы.',
                            'en' => 'Driving school course category.',
                            'lt' => 'Vairavimo mokyklos kursu kategorija.',
                            'pl' => 'Kategoria kursow szkoly jazdy.',
                        ],
                    );
                    $description = $this->completeTranslations(
                        $category->getTranslations('description'),
                        [
                            'ru' => 'Категория группирует программы обучения, цены, заявки и публичные страницы курсов автошколы.',
                            'en' => 'This category groups training programs, prices, applications, and public driving school course pages.',
                            'lt' => 'Kategorija grupuoja mokymo programas, kainas, paraiskas ir viesus vairavimo mokyklos kursu puslapius.',
                            'pl' => 'Kategoria grupuje programy szkoleniowe, ceny, zgloszenia i publiczne strony kursow szkoly jazdy.',
                        ],
                    );
                    $seoTitle = $this->completeTranslations(
                        $category->getTranslations('seo_title'),
                        [
                            'ru' => $name['ru'].' | автошкола',
                            'en' => $name['en'].' | driving school',
                            'lt' => $name['lt'].' | vairavimo mokykla',
                            'pl' => $name['pl'].' | szkola jazdy',
                        ],
                    );
                    $seoDescription = $this->completeTranslations($category->getTranslations('seo_description'), $shortDescription);

                    $category->fill([
                        'code' => $category->code ?: Str::snake($category->slug ?: 'category_'.$category->id),
                        ...
                        'image' => $category->image ?: self::IMAGE_PATH,
                        'icon' => $category->icon ?: 'car-front',
                        'is_active' => $category->is_active ?? true,
                        'is_visible_on_site' => $category->is_visible_on_site ?? true,
        ...
        Course::query()
            ...
            ->chunkById(100, function ($courses) use ($fallbackCategory): void {
                foreach ($courses as $course) {
                    ...
                    $course->fill([
                        'code' => $course->code ?: Str::upper(Str::snake($course->slug ?: 'course_'.$course->id)),
                        'title' => $title['en'],
